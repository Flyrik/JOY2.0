...
class Objet:
    ...
    def Maj_object(self, filename):
        object = self.load_from_json(filename)
        pers = Objet(name=self.name or object.name, material=self.material or object.material, forme=self.forme or object.forme, usage=self.usage or object.usage, genre=self.genre or object.genre, taille=self.taille or object.taille)
    ...
    def Info(self, filename):
        ...
        if object.usage is not None:
            if object.name is None:
                presentation += f" {pronom_personal}"
            presentation += f" sert à {object.usage},"
...
if __name__ == "__main__":
    Objet().Maj_feature(list_feat=["taille"])
